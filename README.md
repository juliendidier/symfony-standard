...
Elle permet de :
 - Définir une commande (nom, description arguments, options)
    ```Command::configure()```

 - Executer une commande
    ```Command:execute(InputInterface, OutputInterface)```
...
Elle permet de :
 ...
 - Rendre les commandes executables en ligne de commandes
    ```Application::run()```
...
```php ./test.php``` retourne la liste des commades
...
Il existe différents modes (```$mode```), permettant de :
 ...
 - rendre l'option obligatoire : ```InputOption::VALUE_REQUIRED```
